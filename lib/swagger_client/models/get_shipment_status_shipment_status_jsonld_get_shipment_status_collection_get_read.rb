...
module SwaggerClient
  # Shipment Status API 
  class GetShipmentStatusShipmentStatusJsonldGetShipmentStatusCollectionGetRead
    attr_accessor :id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::GetShipmentStatusShipmentStatusJsonldGetShipmentStatusCollectionGetRead` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::GetShipmentStatusShipmentStatusJsonldGetShipmentStatusCollectionGetRead`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, type, context, unique_id, house_bill, status_code, status_message, status_date, status_time, control_airport, origin, destination, comment, gmt_offset].hash
    end
