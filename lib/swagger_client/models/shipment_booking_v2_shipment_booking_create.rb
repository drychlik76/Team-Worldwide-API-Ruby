...
module SwaggerClient
  ...
  class ShipmentBookingV2ShipmentBookingCreate
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::ShipmentBookingV2ShipmentBookingCreate` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::ShipmentBookingV2ShipmentBookingCreate`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [booked_date, shipping_mode, carrier_name, origin_code, destination_code, vendor_code, carrier_booking_number, carrier_booking_number2, service, direct_to_consignee, override_measures, total_pieces, total_weight, departure_date, departure_time, arrival_date, arrival_time, booking_station, general_notes, shipments].hash
    end
