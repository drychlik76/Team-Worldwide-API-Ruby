...
module SwaggerClient
  ...
  class ShipmentV2ShipmentJsonldShipmentCreate
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::ShipmentV2ShipmentJsonldShipmentCreate` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::ShipmentV2ShipmentJsonldShipmentCreate`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [context, id, type, is_test_shipment, house_bill, control_branch, company, shipment_type, service, pickup_date, origin, destination, linear_units, weight_units, oversize, goods_description, weight, special_instructions, declared_value, shipper, consignee, bill_to, requested_pickup, requested_delivery, shipment_goods, shipment_goods_handling, shipment_notes, additional_references, projected_expenses, projected_customer_charges].hash
    end
